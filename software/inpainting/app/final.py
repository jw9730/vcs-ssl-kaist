# pylint: disable=line-too-long,no-value-for-parameter,protected-access,too-many-locals
import glob
import random
import json
...
def parse_scene_graph(img_file, masked_img_file, json_file, json_sample_file):
    # label info
    ...
    with open(json_sample_file, 'r', encoding='utf-8') as f:
        ...
        masked_object = json_data['masked_object']
    ...
    # parse image
    img = adjust_exif_orientation(Image.open(img_file))
    masked_img = adjust_exif_orientation(Image.open(masked_img_file))
...
def main(split='val', vis=False):
    ...
    data_dir = f'../test_data_KETI_SG_DB/{split}'
    result_img_dir = Path(f'../test_results_KETI_SG_DB/{split}/img_outputs')
    result_json_dir = Path(f'../test_results_KETI_SG_DB/{split}/json_outputs')
    result_img_dir.mkdir(exist_ok=True, parents=True)
    result_json_dir.mkdir(exist_ok=True, parents=True)

    img_files = []
    masked_image_files = []
    json_files = []
    json_sample_files = []
    for file_id in tqdm.tqdm([Path(f).stem for f in glob.glob(f'{data_dir}/json/*.json')]):
        for json_sample_file in sorted(glob.glob(f'{data_dir}/json_samples/{file_id}_*.json')):
            img_files.append(f'{data_dir}/image/{file_id}.jpg')
            masked_image_files.append(f'{data_dir}/masked_image/{Path(json_sample_file).stem}.jpg')
            json_files.append(f'{data_dir}/json/{file_id}.json')
            json_sample_files.append(json_sample_file)
    for img_file, masked_img_file, json_file, json_sample_file in tqdm.tqdm(zip(img_files, masked_image_files, json_files, json_sample_files), total=len(img_files)):
        data, img, masked_img, image_id, image_name = parse_scene_graph(img_file, masked_img_file, json_file, json_sample_file)
        output_id = Path(json_sample_file).stem
        if vis:
            # original
            img.show()
            ImageDraw.Draw(img)
            visualize_scene_graph(data, img, show_mask=False)
            plt.savefig(result_img_dir / f'{output_id}.png', dpi=200)
            # masked
            masked_img.show()
            ImageDraw.Draw(masked_img)
            visualize_scene_graph(data, masked_img, show_mask=True)
            plt.savefig(result_img_dir / f'{output_id}_masked.png', dpi=200)
        # predict
        ...
        if vis:
            ...
            ImageDraw.Draw(masked_img)
            visualize_scene_graph(data, masked_img, show_mask=True)
            plt.savefig(result_img_dir / f'{output_id}_output.png', dpi=200)
            # close all figures
            plt.close('all')
        else:
            json_data = scene_graph_to_json_dict(data, masked_img, image_id, image_name)
            with open(result_json_dir / f'{output_id}_output.json', 'w', encoding='utf-8') as f:
